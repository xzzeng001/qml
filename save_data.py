...
def save_execution_info(simulator: Optional[bool] = None,
                        ...
                        num_entangle: Optional[int]=None,
                        required_qubits: Optional[Any] = None,
                        shots: Optional[int] = None,
                        epochs: Optional[int] = None,
                        optimize_method: Optional[str] = None,
                        order_of_derivative: Optional[int] = None, 
                        device_name: Optional[str] = None,
                        simulation_method: Optional[str]=None,
                        analy_grad: Optional[bool] = None,
                        load_params_from: Optional[str] = None,
                        optimization_level: Optional[int] = None,
                        adapt_vqe: Optional[bool]=None,
                        adapt_tol: Optional[float]=None,
                        adapt_max_iter: Optional[float]=None,
                        measurement_method: Optional[str] = None,
                        transpilation_info: Optional[str] = None,
                        num_params: Optional[int] = None,
                        timestamp: Optional[str] = None,
                        grouping: Optional[bool]=None,
                        number_of_hamiltonian: Optional[int]=None,
                        MeanSquareError: Optional[float]=None,
                        number_of_groups: Optional[int]=None) -> None:
    """
...
def draw_circuit(circ: Any, filename: str='circuit.png') -> None:
    ...
    """
    try:
        fig = circ.draw(output=draw_method, filename='output/{}/{}'.format(TIMESTAMP,filename))
        if draw_method == 'mpl':
            plt.close(fig)
    except:
...
def save(ansatz: Optional[Any] = None, 
    ...
    if (all_params_epochs and ansatz): 
        np.savetxt("output/{}/params.txt".format(TIMESTAMP),all_params_epochs)
#        save_all_params_epochs(all_params_epochs)
#        if ansatz.name == 'dqnn':
#            plot_all_params_epochs(ansatz, all_params_epochs)
    
    if (plot_list_cost): 
        np.savetxt("output/{}/energy.txt".format(TIMESTAMP),plot_list_cost)
...
def save_and_plot_energy(plot_list_energy,
                         filename: str = "r_energy.pdf") -> None:
    """
    Plot the distance versus FCI and outcome energy.
    """
    iflag=False
    if os.path.isfile('output/r_energy.txt'):
        iflag=True

    with open('output/r_energy.txt','a') as f: 
        np.savetxt(f,plot_list_energy)          
  
    if iflag:
        plot_energy = np.loadtxt('output/r_energy.txt',dtype=float)
    else:
        plot_energy = np.array(plot_list_energy)

    plt.figure()
    plt.plot(plot_energy[:,0], plot_energy[:,1], "--o", color="r", label="FCI", ms=4)
    plt.plot(plot_energy[:,0], plot_energy[:,2], "--o", color="b", label="QML", ms=4)
    plt.xlabel("Epoch")
...
def find_minimum()->float:
    filename="energy.txt"
    rr_all=np.asarray(np.loadtxt("output/{}/{}".format(TIMESTAMP, filename)))
    ii=np.argmin(abs(rr_all[:,0]-rr_all[0,1]))
...
def plot_all_params_epochs(ansatz: Any,
    ...
    try:
#        epoch_list = [param[0] for param in all_params_epochs]
        param_list = [param[0] for param in all_params_epochs]
        params_per_epoch: List[List[float]] = [[param_list[i][j] for i in range(len(param_list))] for j in range(len(param_list[0]))]
        reshaped_params: List[List[dict]] = ansatz.reshape_params_for_plotting(params_per_epoch)
        ...
        for l in range(len(reshaped_params)):
            ...
            for j in range(len(reshaped_params[l])):
                ...
                for k, param in enumerate(reshaped_params[l][j]["params"]):
                    new_ax.plot(param, label="Param #{}".format(k+1))
...
def plot_cost_vs_error_probability(csv_filename: str = "test_different_error_probabilities.csv",
    ...
    for t, timestamp in enumerate(timestamps):
        with open("output/{}/execution_info.txt".format(timestamp), "r") as f:
            ...
            for c in content:
                if c.split(":")[0] == "ansatz":
                    ansatz_i = int("QAOA" in c.split(":")[1])
