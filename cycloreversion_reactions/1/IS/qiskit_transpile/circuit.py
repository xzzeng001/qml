...
def HAA_circuit(network,ncycle):
    ...
    num_params=3*network[0]*2+ncycle*3*network[0]*network[1]
...
def HAA_circuit_simplify(network,ncycle):

    num_qubits=sum(network)
    num_params=2*2*network[0]+4*ncycle*network[0]*network[1]
    ...
    for ii in range(ncycle):
       for i in range(network[1]):
          for j in range(network[0]):
             circ.rx(2*params[iparams], q_reg[j])
             circ.rz(2*params[iparams+1], q_reg[j])
             circ.cnot(q_reg[sum(network[:1])+i], q_reg[j])
             circ.rx(2*params[iparams+2], q_reg[sum(network[:1])+i])
             circ.rz(2*params[iparams+3], q_reg[sum(network[:1])+i])         
             iparams += 4
